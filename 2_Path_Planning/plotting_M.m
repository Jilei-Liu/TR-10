...
scatter3(x(size(x,1),1),x(size(x,1),2),x(size(x,1),3),1500,"black","filled");
line(X_string',Y_string',Z_string','Color','black','LineWidth',0.8); 

for i = 1:size(x,1)-1
    nodemark = num2str(i);
    nodemark = strcat('$$\',32,nodemark,'$$');
    te = text(x(i,1),x(i,2),x(i,3),nodemark,"FontSize",18,'Interpreter','latex');
    scatter3(x(i,1),x(i,2),x(i,3),10,"red","filled");
end 
